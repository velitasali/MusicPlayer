...
public class TaggerUtils {
    ...
    static DocumentFile getDocumentFile(Uri treeUri, final File file) {
        ...
        DocumentFile document = DocumentFile.fromTreeUri(RetroApplication.getInstance(), treeUri);
        ...
        try {
            ...
            if (externalFilesDirs != null && externalFilesDirs.length > 0) {
                for (File file : externalFilesDirs) {
                    if (file != null && !file.equals(RetroApplication.getInstance().getExternalFilesDir("external"))) {
