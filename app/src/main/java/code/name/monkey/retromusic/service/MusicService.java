...
import code.name.monkey.backend.helper.StopWatch;
...
import code.name.monkey.backend.providers.HistoryStore;
import code.name.monkey.backend.providers.MusicPlaybackQueueStore;
...
import static code.name.monkey.backend.RetroConstants.ACTION_PLAY_PLAYLIST;
...
import static code.name.monkey.backend.RetroConstants.APP_WIDGET_UPDATE;
import static code.name.monkey.backend.RetroConstants.EXTRA_APP_WIDGET_NAME;
import static code.name.monkey.backend.RetroConstants.INTENT_EXTRA_PLAYLIST;
import static code.name.monkey.backend.RetroConstants.INTENT_EXTRA_SHUFFLE_MODE;
...
import static code.name.monkey.backend.RetroConstants.MUSIC_PACKAGE_NAME;
...
import static code.name.monkey.backend.RetroConstants.RETRO_MUSIC_PACKAGE_NAME;
                    ...
                    case ACTION_PLAY_PLAYLIST:
                        ...
                        if (playlist != null) {
                            ...
                            if (playlist instanceof AbsCustomPlaylist) {
                                ((AbsCustomPlaylist) playlist).getSongs(getApplicationContext()).subscribe(this::setPlaylistSongs);
                                playlistSongs = getPlaylistSongs();
                            } else {
                                //noinspection unchecked
                                PlaylistSongsLoader.getPlaylistSongList(getApplicationContext(), playlist.id).subscribe(songs -> {
                                    setPlaylistSongs((ArrayList<Song>) (List) songs);
                                });
                                playlistSongs = getPlaylistSongs();
